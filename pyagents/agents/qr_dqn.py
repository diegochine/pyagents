...
class QRDQNAgent(DQNAgent):
    """An agent implementing a DQN algorithm based on the QR DQN algorithm (https://arxiv.org/pdf/1707.06887.pdf).
    ...
    def __init__(self,
        ...
        # midpoints (cumulative probs), reshape them for broadcasting
        self._tau_hats = tf.reshape((tf.range(0., self._n) + 0.5) / self._n, (1, -1, 1))
        self._huber_loss = tf.keras.losses.Huber(delta=kappa, reduction=tf.keras.losses.Reduction.NONE)
    ...
    def _loss(self, memories, weights=None):
        ...
        # compute huber loss
        huber_loss = self._huber_loss(target_quantiles[..., None], current_quantiles[..., None])
        quantile_huber_loss = tf.where(tf.less_equal(errors, 0.0),
                                       tf.abs(1. - self._tau_hats) * huber_loss,
                                       tf.abs(self._tau_hats) * huber_loss)

        td_residuals = tf.reduce_sum(tf.reduce_mean(quantile_huber_loss, axis=2), axis=1)
